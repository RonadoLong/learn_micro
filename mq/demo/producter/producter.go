...
func (p *Producer) SetExchangeDeclareAndBindQueueDeclare() *Producer {
	...
	err = p.channel.QueueBind(ctpQueue, ctpRouteKey, exchange, false, nil)
	log.Println(err)
	return p
}

func (p *Producer) SetQueueDeclare() *Producer {
	_, err := p.channel.QueueDeclare(
		ctpQueue,
		true,  // Durable
		false, // Delete when unused
